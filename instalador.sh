...
#      NumLock ligado no xinitrc, Brave+Tor no primeiro boot (o Brave e'
#      necessario pros cookies do YouTube, ver aviso no fim deste script) e
#      fallback de xdotool/wmctrl caso o passo 7 nao tenha resolvido.
...
# ----------------------------------------------------------------------
# 8. Customizacoes de sistema do bar (idioma, fuso, NumLock, Brave+Tor no
#    primeiro boot) -- escrito originalmente como script solto, colado
#    aqui pra rodar sozinho com o resto da instalacao. batocera-save-
#    overlay fica so no passo 9 (nao duplicado aqui) pra cobrir tudo de
#    uma vez so, com o tratamento de erro que ja existe la.
#
#    [FIX 2026-09-12] A condicao original checava "! -d
#    /userdata/system/.dev/apps" pra decidir se instalava o Brave -- so
#    que essa pasta e' criada pelo PASSO 2 deste mesmo instalador, ANTES
#    deste passo rodar. Colado como estava, o Brave/Tor nunca dispararia
#    de verdade numa maquina que passou por este instalador. Trocado pra
#    checar o proprio Brave (nome do binario pode ser "brave-browser" ou
#    "brave" dependendo de como aquele instalador de terceiros o deixa).
# ----------------------------------------------------------------------
...
if ! command -v brave-browser >/dev/null 2>&1 && ! command -v brave >/dev/null 2>&1; then\n\
    # Instala o Navegador Brave com atalho para o TOR caso ele ainda nao esteja instalado --\n\
    # em segundo plano e com timeout: sem isso, rede lenta ou o link fora do ar bem no boot\n\
    # (bit.ly costuma retornar erro as vezes) travava o xinitrc inteiro esperando o curl,\n\
    # atrasando openbox/jukebox por minutos -- so a instalacao do Brave em si atrasa, nao o boot.\n\
    (curl -fsSL --connect-timeout 3 --max-time 20 bit.ly/JCGAMES-TOR | bash > /dev/null 2>&1) &\n\
fi\n\
\n\
# Verificar se o Num Lock esta desligado (off)\n\
if xset q | grep -q "Num Lock:.*off"; then\n\
    ...
    log "xinitrc: bloco de Brave/Tor + NumLock adicionado (roda no proximo boot)"
else
    log "xinitrc: bloco de Brave/Tor + NumLock ja presente ou $XINITRC nao encontrado -- pulei"
fi
