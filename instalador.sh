...
#   3. Coloca yt-dlp/deno/bgutil-pot (vem dentro do squashfs) em
#      /userdata/system/.local/bin, e instala o Brave (+ atalho Tor,
#      tambem vendorizado no squashfs) direto -- sem baixar nada (o Brave
#      logado e' necessario pros cookies do YouTube, ver aviso no fim
#      deste script). So instala o Brave se ainda nao existir na maquina.
...
#   8. Customizacoes de sistema do bar: idioma pt_BR, fuso America/Sao_Paulo,
#      NumLock ligado no xinitrc (e remove o bloco antigo de Brave/Tor via
#      curl se a maquina tiver instalado antes dessa parte virar o passo
#      3), e fallback de xdotool/wmctrl caso o passo 7 nao tenha resolvido.
#   9. Salva o overlay no disco (batocera-save-overlay) -- ESSENCIAL: a
#      raiz do Batocera e' um overlay em RAM, os passos 6, 7 e 8 mexem fora
...
# [2026-09-12] XINITRC e BATOCERA_CONF nao eram parametrizaveis -- uma
# instalacao "seca" com as variaveis acima sobrescritas ainda mexia de
# verdade nesses dois arquivos do SISTEMA (fora de qualquer JUKEBOX_DIR
# de teste) e chamava batocera-save-overlay de verdade, persistindo tudo.
# Reproduzido sem querer nesta maquina de dev ao testar o pacote de hoje.
...
# [2026-09-12] Brave (+ atalho Tor), vendorizado dentro do squashfs (ver
# empacotar.sh) -- so copia pro lugar final, sem baixar nada. Antes disso
# a instalacao dependia de um script de terceiro rodando no xinitrc a
# CADA boot (bit.ly/JCGAMES-TOR), que nunca conseguia detectar "ja
# instalado" direito e reinstalava (~160MB) toda vez, abrindo uma janela
# por cima do jukebox -- ver LEIA-ME.md pro caso completo. So instala se
# BRAVE_APPS_DIR ainda nao existir, pra nao pisar num perfil/login que ja
# tenha sido feito na maquina.
...
if [ -d "$EXTRAIDO/vendor/brave-app" ]; then
    if [ -d "$BRAVE_APPS_DIR" ]; then
        ...
        cp -rf "$EXTRAIDO/vendor/brave-app" "$BRAVE_APPS_DIR"
    ...
    mkdir -p "$DESKTOP_DIR"
    if [ -f "$EXTRAIDO/vendor/brave-desktop/brave.desktop" ]; then
        cp -f "$EXTRAIDO/vendor/brave-desktop/brave.desktop" "$DESKTOP_DIR/brave.desktop"
        chmod 777 "$DESKTOP_DIR/brave.desktop"
    fi
    if [ -f "$EXTRAIDO/vendor/brave-desktop/tor.desktop" ]; then
        cp -f "$EXTRAIDO/vendor/brave-desktop/tor.desktop" "$DESKTOP_DIR/tor.desktop"
...
#    batocera-save-overlay fica so no passo 9 (nao duplicado aqui) pra
#    cobrir tudo de uma vez so, com o tratamento de erro que ja existe la.
#
#    [2026-09-12] O Brave/Tor no boot (curl pra bit.ly/JCGAMES-TOR) saiu
#    daqui -- Brave agora e' vendorizado e instalado no passo 3, direto,
#    sem depender de rede nem rodar em todo boot (ver historico do bug no
#    LEIA-ME.md). Se a maquina ja tiver o bloco antigo (de uma instalacao
#    anterior a esta mudanca), remove ele agora pra parar de tentar baixar
#    e de abrir janela por cima do jukebox.
# ----------------------------------------------------------------------
if [ -f "$XINITRC" ] && grep -qF "JCGAMES-TOR" "$XINITRC"; then
    python3 - "$XINITRC" <<'PYEOF'
import re
import sys
caminho = sys.argv[1]
with open(caminho, "r", encoding="utf-8") as f:
...
padrao = re.compile(
    ...
    r".*?JCGAMES-TOR.*?echo \"Num Lock ja esta ativado\.\"\n+fi\n",
    re.DOTALL)
nova, trocas = padrao.subn("\n", conteudo)
if trocas:
    with open(caminho, "w", encoding="utf-8") as f:
        f.write(nova)
    print("removido")
PYEOF
    log "xinitrc: bloco antigo de Brave/Tor via bit.ly removido (Brave ja vem instalado pelo passo 3 agora)"
fi
...
if [ ! -f /usr/bin/wmctrl ]; then
    if wget -q https://github.com/JeversonDiasSilva/configs/releases/download/v.1.0/wmctrl -O /usr/bin/wmctrl; then
        ...
        log "wmctrl instalado de JeversonDiasSilva/configs (fallback do passo 7)"
    else
...
if [ ! -f /usr/bin/xdotool ]; then
    if wget -q https://github.com/JeversonDiasSilva/configs/releases/download/v.1.0/xdotool -O /usr/bin/xdotool; then
        ...
        log "xdotool instalado de JeversonDiasSilva/configs (fallback do passo 7)"
    else
...
# ----------------------------------------------------------------------
# 9. Salvar o overlay no disco -- CRITICO: a raiz do Batocera e' um
#    overlay em RAM (tmpfs) por cima de um squashfs so-leitura (ver "mount"
#    -- / e' "overlay", so /userdata e' particao de verdade). Os passos 6
#    (desvio do EmulationStation), 7 e 8 (symlinks/binarios em $BIN_DEST,
#    xinitrc, batocera.conf) mexem FORA de /userdata -- sem salvar agora,
#    tudo isso some no proximo reboot e a maquina volta a abrir o
#    EmulationStation puro no ponto do cliente.
# ----------------------------------------------------------------------
...
log "  python3 $JUKEBOX_DIR/vps_setup.py registrar   # sem flag nenhuma -- pergunta URL da VPS, admin-token (seu, escondido), nome do ponto e se quer ja criar o dono (login/senha do painel /painel-dono). Voce quem roda, nunca o cliente."
log ""
